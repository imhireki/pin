...
@pytest.mark.parametrize("validity, data, is_stored", [
    [True, {"url": "url"}, False],
    [True, {"url": "url"}, True],
    [False, {}, False],
])
def test_store_valid_pins(mocker, validity, data, is_stored):
    storage = mocker.Mock()
    storage.query_pin.return_value = None if not is_stored else "url"
